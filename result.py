...
class resultClass:
    def __init__(self,root):
        ...
        lbl_marks_ob=Label(self.root,text="Marks",font=("goudy old style",20,'bold'),bg='white').place(x=50,y=280) 
        ...
        #----Button----
        btn_add=Button(self.root,text="Submit",font=("times new roman",15,"bold"),bg="#5ed762",activebackground="#74da78", cursor="hand2",command=self.add).place(x=300,y=420,width=120,height=35)
        btn_clear=Button(self.root,text="Clear",font=("times new roman",15,"bold"),bg="#d5dade",activebackground="#cfd2d4", cursor="hand2",command=self.clear).place(x=430,y=420,width=120,height=35)

        #----image----
        self.bg_img=Image.open("images/result.jpg")
        self.bg_img=self.bg_img.resize((550,350),Image.Resampling.LANCZOS)
    ...
    def add(self):
        ...
        try:
            if self.var_name.get()=="":
                ...
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Result is All Ready Present!",parent=self.root)
                else:
                    per = round((int(self.var_marks.get()) * 100) / int(self.var_full_marks.get()), 2)
