...
class Register:
    def __init__(self, root):
        ...
        self.root.geometry("1480x870+0+0")

        screen_height = self.root.winfo_screenheight()

        # ---- Background Image ----
        img = Image.open("images/bg_login.jpg")
        img = img.resize((1230, screen_height), Image.LANCZOS)
        self.bg_img = ImageTk.PhotoImage(img)

        self.bg = Label(self.root, image=self.bg_img)
        self.bg.place(x=250, y=0, height=screen_height)

        # ---- Front Image ----
        front_img = Image.open("images/front.jpg")
        front_img = front_img.resize((500, 500), Image.LANCZOS)
        self.front = ImageTk.PhotoImage(front_img)

        front_label = Label(self.root, image=self.front)
        front_label.place(x=80, y=200)   


        #----Register Frame-----
        frame1=Frame(self.root,bg="white")
        frame1.place(x=582,y=200,width=700,height=504)
        ...
        cpassword=Label(frame1,text="Conform password!",font=("times new roman",15, "bold"), bg="white", fg="black").place(x=370,y=310)
        self.txt_cpassword=Entry(frame1,font=("times new roman", 15),bg="lightgray")
        self.txt_cpassword.place(x=370, y=340, width=250)

        #------------terms & cons-------
        self.var_chk=IntVar()
        chk=Checkbutton(frame1,text="I Agree With The Terms & Condations", variable=self.var_chk, onvalue=1, offvalue=0, bg="white", font=("times new roman", 12)).place(x=50,y=380)

        self.btn_img=ImageTk.PhotoImage(file="images/register.png")
        btn_register=Button(frame1,image=self.btn_img, bd=0, cursor="hand2",command=self.register_data).place(x=50, y=430)
    ...
    def register_data(self):
        if self.txt_fname.get()=="" or self.txt_contact.get()=="" or self.txt_email.get()=="" or self.cmb_quest.get()=="select" or self.txt_answer.get()=="" or self.txt_password.get()=="" or self.txt_cpassword.get()=="":
            ...
            try:
                ...
                cur.execute(
                    "Select * from users where email=?", (self.txt_email.get(),)
                )
                row=cur.fetchone()
                if row!=None:
